...
app.post('/api/clients', authMiddleware, async (req, res) => {
  try {
    const { name, email, phone, company, businessType, platforms, prompt, notes, api_key, api_model, gemini_api_key, whisper_api_key, meta_token, meta_page_id, meta_verify_token, bot_capabilities, pricing, auto_images } = req.body;
    ...
    const client = await createClient({
      ...
      notes, api_key, api_model, gemini_api_key, whisper_api_key, meta_token, meta_page_id, meta_verify_token, bot_capabilities, pricing, auto_images
    });
...
app.put('/api/clients/:id', authMiddleware, async (req, res) => {
  try {
    const { name, email, phone, company, businessType, platforms, prompt, notes, api_key, api_model, gemini_api_key, whisper_api_key, meta_token, meta_page_id, meta_verify_token, bot_capabilities, pricing, catalog, auto_images } = req.body;
    const client = await updateClient(req.params.id, {
      name, email, phone, company,
      business_type: businessType,
      platforms, prompt, notes, api_key, api_model, gemini_api_key, whisper_api_key, meta_token, meta_page_id, meta_verify_token, bot_capabilities, pricing, catalog, auto_images
    });
    if (!client) return res.status(404).json({ error: 'Client non trouvé.' });
    res.json(client);
  } catch (err) {
