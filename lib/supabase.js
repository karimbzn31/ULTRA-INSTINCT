...
export async function uploadProductImage(buffer, filename, mimeType) {
  try {
    ...
    console.log(`[Supabase] ✅ Image uploadée: ${publicUrl}`);
    return publicUrl;
  } catch (err) {
    console.error('[Supabase] Erreur upload:', err.message);
    return null;
  }
}
...
export async function createClient(clientData) {
  const { data, error } = await supabase.from('clients').insert([{
    ...
    auto_images: clientData.auto_images || 'on_request',
...
export async function updateClient(id, updates) {
  const clean = {};
  const allowed = ['name', 'email', 'phone', 'company', 'business_type', 'platforms', 'prompt', 'notes', 'catalog', 'logo', 'catalog_filename', 'api_key', 'api_model', 'gemini_api_key', 'whisper_api_key', 'meta_token', 'meta_page_id', 'meta_verify_token', 'bot_capabilities', 'pricing', 'auto_images', 'google_sheet_id', 'google_sheet_service_key'];
